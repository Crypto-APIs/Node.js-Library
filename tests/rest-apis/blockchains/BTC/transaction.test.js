async function Transaction(caClient) {
    ...
    const chainInfo = await caClient.BC.BTC.blockchain.getInfo().then(response => response.payload); // Duplicated but needed for scenario
    const transactions = await caClient.BC.BTC.transaction.getTransactionIndexByBlock(chainInfo.bestBlockHash).then(response => response ? response.payload : null);
    ...
    await caClient.BC.BTC.transaction.getUnconfirmedTransactions();
    await caClient.BC.BTC.transaction.getTransactionsFee();
    ...
    await caClient.BC.BTC.transaction.decodeRawTransaction(txHex).then(response => response.payload);
    ...
    await caClient.BC.BTC.transaction.createTransaction(txIns, txOuts, fee);
}
