async function Transaction(caClient) {
    ...
    const chainInfo = await caClient.BC.DOGE.blockchain.getInfo().then(response => response.payload); // Duplicated but needed for scenario
    const block = await caClient.BC.DOGE.blockchain.getBlockByHeight(chainInfo.blocks - 6).then(response => response.payload); // Duplicated but needed for scenario
    const transactions = await caClient.BC.DOGE.transaction.getTransactionIndexByBlock(block.hash).then(response => response ? response.payload : null);
    ...
    await caClient.BC.DOGE.transaction.getUnconfirmedTransactions();
    await caClient.BC.DOGE.transaction.getTransactionsFee();
    ...
    await caClient.BC.DOGE.transaction.decodeRawTransaction(txHex).then(response => response.payload);
}
