async function PaymentForwarding(caClient) {
    ...
    const addresses = await caClient.BC.BCH.wallet.generateAddressInHDWallet(randomHDWalletName, 1, hdWalletPass).then(response => response.payload.addresses); // Duplicated but needed for scenario
