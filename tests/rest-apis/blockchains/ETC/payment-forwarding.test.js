async function PaymentForwarding(caClient) {
    ...
    const payment = await caClient.BC.ETC.paymentForwarding.createPaymentForwarding(testUrl, from, to, privateKey, confirmations).then(res => res.payload);

    await caClient.BC.ETC.paymentForwarding.deletePayment(payment.uuid);
}
