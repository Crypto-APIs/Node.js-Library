async function Transaction(caClient) {
    ...
    const chainInfo = await caClient.BC.BCH.blockchain.getInfo().then(response => response.payload); // Duplicated but needed for scenario
    const transactions = await caClient.BC.BCH.transaction.getTransactionIndexByBlock(chainInfo.bestBlockHash).then(response => response ? response.payload : null);
    ...
    await caClient.BC.BCH.transaction.getUnconfirmedTransactions();
    await caClient.BC.BCH.transaction.getTransactionsFee();
    ...
    await caClient.BC.BCH.transaction.decodeRawTransaction(txHex).then(response => response.payload);
    ...
    await caClient.BC.BCH.transaction.createTransaction(txIns, txOuts, fee);
}
