async function Transaction(caClient) {
    console.log('\n::DASH Transaction');

    const chainInfo = await caClient.blockchain.DASH.blockchain.getInfo().then(response => response.payload); // Duplicated but needed for scenario
    const transactions = await caClient.blockchain.DASH.transaction.getTransactionIndexByBlock(chainInfo.bestBlockHash).then(response => response.payload);

    if (transactions && transactions.length) {
        const txId = transactions[0].txid;
        await caClient.blockchain.DASH.transaction.getTransaction(txId);
    }
    await caClient.blockchain.DASH.transaction.getUnconfirmedTransactions();
    await caClient.blockchain.DASH.transaction.getTransactionsFee();

    const txHex = '';
    await caClient.blockchain.DASH.transaction.decodeRawTransaction(txHex).then(response => response.payload);

    const txIns = [{address: 'yVrjEE1zwXckCMXZoTStJtb9SJ29xr1ZMc', value: 121.309}];
    const txOuts = [{address: 'yMNjY5gZs5RwUovbL1NzXjbnkgPMYfUA98', value: 121.309}];
    const fee = {value: 0.00000001};

    await caClient.blockchain.DASH.transaction.createTransaction(txIns, txOuts, fee);
    // await caClient.blockchain.DASH.transaction.signTransaction(); // TODO
    // await caClient.blockchain.DASH.transaction.sendTransaction(); // TODO
    // await caClient.blockchain.DASH.transaction.newTransaction();
    // await caClient.blockchain.DASH.transaction.createHDWalletTransaction(); // TODO
}


module.exports = Transaction;
