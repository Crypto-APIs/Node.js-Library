async function Transaction(caClient) {
    console.log('\n::DOGE Transaction');

    const chainInfo = await caClient.BC.DOGE.blockchain.getInfo().then(response => response.payload); // Duplicated but needed for scenario
    const transactions = await caClient.BC.DOGE.transaction.getTransactionIndexByBlock(chainInfo.bestBlockHash).then(response => response ? response.payload : null);

    if (transactions && transactions.length) {
        const txId = transactions[0].txid;
        await caClient.BC.DOGE.transaction.getTransaction(txId);
    }

    await caClient.BC.DOGE.transaction.getUnconfirmedTransactions();
    await caClient.BC.DOGE.transaction.getTransactionsFee();

    const txHex = '';
    await caClient.BC.DOGE.transaction.decodeRawTransaction(txHex).then(response => response.payload);

    const txIns = [{address: 'no3yJMxBSKzq6wuUNLN7cUssfPGTiRbb5c', value: 121.309}];
    const txOuts = [{address: 'nsXYgWCuBVSYxD1rWz543EFkfxcPV9PC2y', value: 121.309}];
    const fee = {value: 0.00000001};

    await caClient.BC.DOGE.transaction.createTransaction(txIns, txOuts, fee);
}


module.exports = Transaction;
